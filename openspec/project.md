...
1. **Proposal Phase**: Create OpenSpec proposal with requirements and test scenarios using the `spec-component` skill
2. **Approval Gate**: Proposal must be approved before implementation begins
3. **Implementation Phase**: Implement the component using the `develop-component` skill, which uses scaffdog automation (`bun run scaffold:component '<json>'`) to generate boilerplate, then refines following tasks.md checklist (~70% time savings)
4. **Archiving Phase**: Archive proposal after merge to create final spec

See `.claude/skills/spec-component/SKILL.md` and `.claude/skills/develop-component/SKILL.md` for the complete workflow, templates, and usage examples.
